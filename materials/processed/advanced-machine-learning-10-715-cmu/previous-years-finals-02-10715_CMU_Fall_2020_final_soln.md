# previous-years-finals-02-10715_CMU_Fall_2020_final_soln

Source: `materials/archive/advanced-machine-learning-10-715-cmu/piazza-resources/previous-years-finals-02-10715_CMU_Fall_2020_final_soln.pdf`
Duplicate equivalents: `previous-years-finals-02-10715_CMU_Fall_2020_final_soln.pdf`
...
### Visual Description
A standard exam cover page containing the course title, due date, a list of instructions in bullet points, and a table for the distribution of marks across seven questions.
...
Consider $\mathcal{X} = \{-1, 1\}^2$ and $\mathcal{Y} = \{-1, 1\}$. Consider the following three functions: AND, OR and NOT.
...
Consider a neuron with weights $\mathbf{w}$ and bias $b$. And its transformation $f : \mathcal{X} \to \mathbb{R}$, with $f(\mathbf{x}) = \text{sign}(\mathbf{w}^\top \mathbf{x} + b)$.

(a) (3 points) Show there exists a neuron with the behavior of the NOT function.

(b) (3 points) Show there exists a neuron with the behavior of the AND function.

(c) (4 points) Show there exists a neuron with the behavior of the OR function.
...
### Visual Description
Text-only slide with mathematical definitions of Boolean functions (AND, OR, NOT) using piecewise notation and a single neuron model.
...
![Figure 1: Model](figure1.png)
*Figure 1: Model*

It might be wrong because Dr. X. is not accounting for youth, a confounding variable. Being young affects both the compliance of taking the drug and the probability of recovery, so a causal relation cannot be correctly estimated without accounting for youth.

To show this, consider the following case:
* $\mathbb{P}(M = 1|Y = 1) = 1$
* $\mathbb{P}(M = 1|Y = 0) = 0$
* $\mathbb{P}(H = 1|Y = 1, M = 1) = \mathbb{P}(H = 1|Y = 1, M = 0) = 0.8$
* $\mathbb{P}(H = 1|Y = 0, M = 1) = \mathbb{P}(H = 0|Y = 0, M = 0) = 0.5$

In this case all young people take the medicine and no old people take the medicine. Moreover, the probability of recovery of youth people is unaffected by the medicine and equal to 0.8. Finally, the probability of recovery of old people is unaffected by the medicine and equal to 0.5.

This case is consistent with the results observed by Dr. X but the medicine has no effect on recovery.

Page 5

### Visual Description
The page contains a solution with a directed acyclic graph (DAG). The graph has three circular nodes: 'Y' at the top, which has arrows pointing down to 'M' (on the left) and 'H' (on the right). There is also a horizontal arrow pointing from 'M' to 'H'. Below the graph is text explaining the confounding effect of 'Y' (youth) with mathematical probability statements.
...
### Visual Description
Text-only slide with a footnote.

---

## Page 7
### Content
**SOLUTION:** The intuition is that even if the algorithm is able to identify the best expert, it may not be able to use their prediction due to the randomness of sampling. Assume the adversary generates the same data for $d$ experts, $E_1, E_2, \dots, E_d$ such that $E_1$ will always have the correct prediction and the rest will always make a mistake. In this case, the best expert in the hindsight will achieve no mistake on any day $T$. Hence the regret at $T$ will simply be the number of mistakes made by the algorithm. Then 

$$
\frac{\mathbb{E}[R(T)]}{T} = \frac{\mathbb{E}[\text{number of mistakes made till } T]}{T} \geq \frac{T \mathbb{P}(E_1 \text{ is not chosen on any given day})}{T}
$$

$$
= \mathbb{P}(E_1 \text{ is not among the two experts chosen on a given day}) = 1 - \frac{d-1}{d(d-1)/2} \geq \frac{1}{3} > 0.
$$

Hence it is not a no-regret algorithm.
...
### Visual Description
Text-only slide with mathematical formulas for square loss minimization and a decision rule, followed by a specific dataset and a problem description.
## Page 9
### Content
SOLUTION: After adversarially changing the point $(-1, 1)$ to $(20, 1)$ the empirical risk minimization algorithm obtains as solution $\hat{w} \approx -0.0423$ and $\hat{b} \approx 0.0889$.

Using this values the prediction of the fitted model for the $x = 2$ will be:
$\hat{f}_n(2, -0.0423, 0.0889) = \text{sign}(0.0042) = 1$, which flips the predicted label.
...
Let $\mathcal{X} = \mathbb{R}^d$ and $\mathcal{Y} = \{1, \dots, K\}$. Consider the training data $S = \{(\mathbf{x}_i, y_i)\}_{i=1}^n$. Each $\mathbf{x}_i$ is drawn i.i.d. from some unknown distribution $\mathcal{D}$.
...
With this context, the likelihood for the data $S$, under the hypothesis class represented by $\theta$, is given by:
$$L(\theta, S) = \prod_{i=1}^n \prod_{k=1}^K \pi_k(\theta, \mathbf{x}_i)^{\mathbb{1}\{y_i=k\}}.$$

On a separate note, we will now introduce you to a loss that is a popular loss used for classification problems. This is called the cross-entropy loss. The (empirical) cross-entropy for the data $S$, under the hypothesis class represented by $\theta$, is given by:
$$E(\theta, S) = -\sum_{i=1}^n \sum_{k=1}^K \mathbb{1}\{y_i = k\} \log(\pi_{y_i}(\theta, \mathbf{x}_i)).$$
...
$$\text{argmax}_\theta L(\theta, S) = \text{argmin}_\theta E(\theta, S)$$
...
### Content
SOLUTION:
$$\begin{aligned} \text{argmax}_\theta L(\theta, S) &= \text{argmin}_\theta - L(\theta, S) \\ &= \text{argmin}_\theta - \log (L(\theta, S)) \\ &= \text{argmin}_\theta - \sum_i^n \sum_{k=1}^K \mathbb{1}\{y_i = 1\} \log (\pi_k(\theta, \mathbf{x}_i)) \\ &= \text{argmin}_\theta - \sum_i^n \sum_{k=1}^K \mathbb{1}\{y_i = k\} \log (\pi_k(\theta, \mathbf{x}_i)) \\ &= \text{argmin}_\theta E(\theta, S) \end{aligned}$$

Page 11
### Visual Description
Text-only slide. Note: The third line of the derivation contains a typo in the source where the indicator function uses $y_i = 1$ instead of $y_i = k$.
...
Let $\mathcal{X} = \mathbb{R}^d$ and $\mathcal{Y} = \mathbb{R}$. Each $\mathbf{x}_i$ is drawn i.i.d. from some unknown distribution $\mathcal{D}$. Let $h : \mathcal{X} \to \mathbb{R}$ be a model that uses all the $d$ features and we denote $h_S : \mathcal{X} \to \mathbb{R}$ to be a model which uses a subset $S$ of the features. Let $F = \{1, \dots, d\}$ be the feature indexes. The Shapley value of a single feature $f \in F$ for a prediction $h(\mathbf{x})$ is denoted by $\phi_f(h(\mathbf{x}))$, and it measures the average difference in the predictions with and without this feature $f$. Formally, the Shapley value of any feature $f$ is defined as:
...
| model | prediction |
| :--- | :--- |
| $h_\emptyset(\mathbf{x}) = h(\bar{x}_1, \bar{x}_2, \bar{x}_3)$ | 1,740 |
| $h_{\{1\}}(\mathbf{x}) = h(x_1, \bar{x}_2, \bar{x}_3)$ | 2,490 |
| $h_{\{2\}}(\mathbf{x}) = h(\bar{x}_1, x_2, \bar{x}_3)$ | 1,790 |
| $h_{\{3\}}(\mathbf{x}) = h(\bar{x}_1, \bar{x}_2, x_3)$ | 1,500 |
| $h_{\{1,2\}}(\mathbf{x}) = h(x_1, x_2, \bar{x}_3)$ | 2,540 |
| $h_{\{2,3\}}(\mathbf{x}) = h(\bar{x}_1, x_2, x_3)$ | 1,550 |
| $h_{\{1,3\}}(\mathbf{x}) = h(x_1, \bar{x}_2, x_3)$ | 2,250 |
| $h_{\{1,2,3\}}(\mathbf{x}) = h(x_1, x_2, x_3)$ | 2,300 |
...
**Learning goal:** Further understand the concept of Shapley values with a practical simple example. A second goal is to understand the computational complexity of calculating Shapley values and potential limitations.

To compute the Shapley value the predictions of $h_S$ and $h_{S \cup \{f\}}$ models are needed for all subsets. While we did a brute force calculation of $\phi_f$ in this toy question, in practice

Page 12
### Visual Description
The slide contains text and a table (Table 1) that lists eight different model configurations (subsets of features) and their corresponding numerical predictions for a house value.

---
## Page 13
### Content
usually only the model with the full set of features $F$ is trained with the training data. Then features are "removed" to make a prediction for $\mathbf{x}$ by replacing their values with their mean for the feature $f$ on the training data, this is denoted by $\bar{x}_f$.

SOLUTION:
$$\begin{aligned} \phi_1(h(\mathbf{x})) &= \frac{1}{3} \left[ \frac{1}{\binom{2}{0}}(2490 - 1740) + \frac{1}{\binom{2}{1}}(2540 - 1790) + \frac{1}{\binom{2}{1}}(2250 - 1500) + \frac{1}{\binom{2}{2}}(2300 - 1550) \right] \\ &= 750 \end{aligned}$$

$$\begin{aligned} \phi_2(h(\mathbf{x})) &= \frac{1}{3} \left[ \frac{1}{\binom{2}{0}}(1790 - 1740) + \frac{1}{\binom{2}{1}}(2540 - 2490) + \frac{1}{\binom{2}{1}}(1550 - 1500) + \frac{1}{\binom{2}{2}}(2300 - 2250) \right] \\ &= 50 \end{aligned}$$

$$\begin{aligned} \phi_3(h(\mathbf{x})) &= \frac{1}{3} \left[ \frac{1}{\binom{2}{0}}(1500 - 1740) + \frac{1}{\binom{2}{1}}(1550 - 1790) + \frac{1}{\binom{2}{1}}(2250 - 2490) + \frac{1}{\binom{2}{2}}(2300 - 2540) \right] \\ &= -240 \end{aligned}$$

The number of terms in summation grows $O(2^{d-1})$ with number of features $d$ since there are $2^{d-1}$ subsets. Becomes computationally intractable for large $d$.

Page 13
### Visual Description
Text-only slide containing mathematical calculations for Shapley values and a brief complexity analysis.
...
Page 14
### Visual Description
The slide contains text explaining Thompson sampling and a pseudocode block for Algorithm 1. There are also two detailed footnotes at the bottom explaining the Beta distribution and conjugate priors.
...
## Page 16
### Content
SOLUTION: Figure 1 shows the implementation of "Thompson sampling" algorithm. The code was included on the github repo.

![actual regret for Thompson sampling](figure_2.png)
Figure 2: Regret vs iteration

Page 16
### Visual Description
The slide contains a plot titled "actual regret for Thompson sampling". The x-axis represents iterations from 0 to 2000, and the y-axis represents regret from -30 to 50. There are three lines: a central solid line representing the mean regret (which stays relatively flat near 10) and two outer lines representing the 5th and 95th quantiles, which diverge over time.
